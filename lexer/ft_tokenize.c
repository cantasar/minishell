...
int	ft_handle_token(t_data *ms, t_lexer **list, int i)
{
	...
	token = ft_get_token(ms->line[i]);
	if (token == GREAT && ft_get_token(ms->line[i + 1]) == GREAT)
	{
		if (!ft_token(">>", GREAT_GREAT, list))
			return (-1);
		return (2);
	}
	else if (token == LESS && ft_get_token(ms->line[i + 1]) == LESS)
	{
	...
	{
		if (!ft_token(ms->line + i, token, list))
			return (-1);
		return (1);
	}
}
